...
test.describe('Checkout', () => {
    test('should show "Please login to checkout" when not logged in', async ({ page }) => {
        await page.goto('/checkout');
        await expect(page.getByRole('heading', { name: 'Please login to checkout' })).toBeVisible();
    });

    test('should show "Your cart is empty" when logged in but cart is empty', async ({ page }) => {
    ...
    test('should show the checkout form with shipping fields', async ({ page }) => {
    ...
    test('should complete checkout and navigate to order confirmation', async ({ page }) => {
        await setupLoggedInUserWithCart(page);
        await page.goto('/checkout');
        ...
        await expect(page).toHaveURL(/\/orders\/\d+\?success=true/, { timeout: 10000 });
    });
    ...
    test('should show order summary on the orders page', async ({ page }) => {
        ...
        await expect(page.locator('.orders-page, .order-card, h1, h2').first()).toBeVisible();
    });
});
